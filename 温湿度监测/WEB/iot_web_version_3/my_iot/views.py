...
def data(request):
    ...
    if 'recv' in request.GET:
        t, h = getValue()
        recentValue=HistoryValue.objects.order_by('-time')[:20]
        avgT=float(recentValue[0].temperature)
        avgH=float(recentValue[0].humidity)
        for value in recentValue: 
            avgT=(avgT+float(value.temperature))/2
            if abs(float(recentValue[19].temperature)-avgT)<2:
                temperature1 = float(recentValue[19].temperature)
            avgH=(avgH+float(value.humidity))/2
        return HttpResponse(json.dumps({'t':t,'h':h,'avg_t':avgT,'avg_h':avgH,'a':temperature1}))
    return HttpResponse("ok")
